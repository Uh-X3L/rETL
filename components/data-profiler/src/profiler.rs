...
pub fn read_dataframe(path: &Path, format: &InputFormat, delimiter: u8) -> Result<DataFrame> {
    ...
    match format {
        InputFormat::Csv | InputFormat::Txt => {
            ...
            reader.finish().context("Failed to read CSV/TXT file")
        }
        InputFormat::Json => JsonReader::new(file)
            .finish()
            .context("Failed to read JSON file"),
        InputFormat::Parquet => ParquetReader::new(file)
            .finish()
...
mod tests {
    ...
    fn test_profile_any_formats() {
        ...
        let csv_path = Path::new(manifest_dir).join("data/examples/sample.csv");
        let json_path = Path::new(manifest_dir).join("data/examples/sample.json");
        let parquet_path = Path::new(manifest_dir).join("data/examples/sample.parquet");
    ...
    fn test_profile_detailed() {
        let manifest_dir = env!("CARGO_MANIFEST_DIR");
        let path = Path::new(manifest_dir).join("data/examples/sample.csv");
